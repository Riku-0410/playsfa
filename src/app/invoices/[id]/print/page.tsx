...
/** 明細が少なくても票面が埋まるよう、この行数までは空行で埋める */
  ...
  return (
    <div className="py-8 print:py-0">
      ...
      {!invoice.invoice_number && (
        <div className="mx-auto mb-6 w-[210mm] max-w-full rounded-inner bg-warn-soft px-5 py-3 text-sm text-warn-deep print:hidden">
          この請求書は未発行のため、請求番号がまだ採番されていません。請求一覧の「発行」を押すと採番されます(このまま印刷した場合、請求番号の行は出力されません)。
        </div>
      )}

      {/* A4用紙 (210mm × 297mm)。印刷時は @page margin:0 でこのシートがそのまま1ページになる */}
      <main className="mx-auto flex min-h-[297mm] w-[210mm] max-w-full flex-col bg-surface px-[16mm] py-[18mm] shadow-card print:min-h-[295mm] print:shadow-none">
        ...
        <div className="mt-10 flex items-start justify-between gap-8">
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-8 text-sm">下記の通りご請求申し上げます。</p>
        ...
        <table className="mt-10 w-full text-sm">
          <thead>
            <tr className="border-y-2 border-ink text-left">
              <th className="py-2.5 pr-4 font-semibold">品目</th>
              <th className="w-40 py-2.5 text-right font-semibold">
                金額(税抜)
              </th>
            </tr>
          </thead>
          <tbody>
            ...
            {Array.from({ length: fillerRows }).map((_, i) => (
              <tr key={`filler-${i}`} className="border-b border-line">
        ...
        <div className="mt-auto space-y-4 pt-12 text-sm">
